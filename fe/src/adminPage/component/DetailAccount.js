...
const DetailAccount = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (idUser !== null) {
                const userApi = await axios.post(`http://localhost:8080/reloadUser?idUser=${idUser}`)
                setUser(userApi.data);

                const orderApi=await axios.get(`http://localhost:8080/order?idUser=${idUser}`)
    ...
    const resetPassword = async () => {
        try {
            const response = await axios.post(`http://localhost:8080/admin/resetPassword?idUser=${user?.id}`)
