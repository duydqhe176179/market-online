...
const AddressForSeller = () => {
    ...
    useEffect(() => {
        ...
        if (!userLocal) {
            ...
            setUser(userLocal);
            if (userLocal.pickupAddress) {
                const [street, ward, district, province] = userLocal.pickupAddress.split(',').map(part => part.trim());
                setStreet(street);
                setWard(ward);
    ...
    const submit = async (e) => {
        ...
        const pickupAddress = `${street},${ward},${district},${province}`;
        const updatedUser = { ...user, pickupAddress: pickupAddress };
        try {
            const response = await axios.post(`${BASE_URL}/user/updateUser`, updatedUser);
            if (response.status === 200) {
                setUser((prevUser) => ({ ...prevUser, pickupAddress }));
                reloadUser(user.id)
    ...
    return (
        <Container style={{ background: "white", padding: "20px" }}>
            <div style={{ borderBottom: "1px solid #F5F5F5", paddingBottom: "20px" }}>
                <div style={{ fontSize: "20px" }}>Địa chỉ lấy hàng (Chỉ dành cho người bán hàng) *</div>
            </div>
...
export default AddressForSeller